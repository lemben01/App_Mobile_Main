...
class BarcodeFragment : Fragment(R.layout.fragment_barcode) {
    ...
    private val quickActiviyLauncher = registerForActivityResult(ScanQRCode(), ::handleQuickieResult)
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        binding.btnZXing.setOnClickListener {
            ...
            options.setPrompt("Veuillez scanner votre code a barres")
            options.setCameraId(0)
            options.setBeepEnabled(true)

            // TODO: Demarrer le scan
            zxingActivityLauncher.launch(options)
        }

        binding.btnQuickie.setOnClickListener{
            quickActiviyLauncher.launch(
    ...
    private fun handleQuickieResult(qrResult: QRResult?) {
        when(qrResult){
            is QRResult.QRSuccess -> binding.txvCodeContent.text = qrResult.content.rawValue
            is QRResult.QRUserCanceled -> binding.txvCodeContent.text = getString(R.string.user_canceled)
            is QRResult.QRMissingPermission -> binding.txvCodeContent.text = getString(R.string.missing_permission)
            is QRResult.QRError -> binding.txvCodeContent.text = qrResult.exception.message
