...
class SearchFragment : Fragment(R.layout.fragment_search) {
    ...
    private fun showMeteo(meteo: Meteo) {
       //TODO: Affichage de chacunes des informations de la meteo
        with(binding){
            ...
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O) {
               ...
               // Instant.ofEpochSecond(meteo.timestamp.toLong()).atZone(ZoneOffset.UTC).plusSeconds(7200)

                txvDateAPI.text = formatter.format(Instant.ofEpochSecond(meteo.timestamp.toLong()).atZone(ZoneOffset.UTC)
